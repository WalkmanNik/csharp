...
namespace ConsoleApp2
{
    ...
    class Class2 : Class1 { } // <- создаем еще один класс и наследуем его от класса "Class1";
    class RedHotChilliPeppers
    {
        static void Main()
        {
            Class1 new_class = new Class2(); // <- мы указываем типом переменной "new_class" класс "Class1"...
                                             // и т.к. "Class1" является родительским для "Class2", то мы вполне спокойно можем проворачивать такую штуку;
            object object1 = "Hello, world!"; // "System.Object" является родительским для всех типов объектов;
            object object2 = 5; // Поэтому мы не указываем здесь какой-то конкретный тип, по типу "int" или "float";
            object object3 = 5.3f;
            Class1 classic = new Class1();
            classic.PrintPLS();
            object new_classic = new Class1();
            // new_classic.PrintPLS(); <- мы не можем использовать методы класса, если ранее мы поместили ссылку на него под тип "object";
            // Т.е. мы можем вызывать методы класса "System.Object", но не можем вызывать методы класса "Class1";
            Class1 old_classic = (Class1)new_classic; // <- используем явное приведение, чтобы поместить ссылку на наш класс в переменную;
            //Class1 old_classic = new_classic; <- такое преобразование небезопасно, и компилятор тупо не даст тебе его сделать;
            old_classic.PrintPLS(); // <- и теперь уже мы можем использовать методы нашего класса "Class1";

            object text = "Some really useful peace of text";
            //Class1 classique = (Class1) text; <- такое явное преобразование вызовет исключение, т.к. тип "string" имеет совершенно иную "архитектуру";
            Class1 classique = text as Class1; // <- в данном случае при использовании ключевого слова "as" идет либо успешное, либо нет преобразование;
            if (classique != null) // <- делаем проверку, нормально ли прошло преобразование...
            {
                classique.PrintPLS(); // <- и если да, то вызываем метод, специфичный конкретно для класса "Class1";
            }

            object object4 = new Class1();
            if (object4 is Class1) // <- благодаря ключевому слову "is" мы можем проверить, действительно ли типом данного объекта является "Class1";
            {                     // Т.к. из-за проверки выше мы явно уверены, что полученный объект уже ТОЧНО является "Class1"...
                Class1 my_object = (Class1)object4; // <- мы используем явное приведение...
                my_object.PrintPLS(); // <- и вызываем специфичный для класса метод;
            }
            if (object4 is Class1 my_object2) // <- мы также можем создать переменную прямо в "if", экономя аж одну строчку кода;
            {                                 // Это создаст переменную и сразу приведет его к указанному типу;
                my_object2.PrintPLS();        // И дальше используя эту переменную мы можем делать вСяКиЕ шТуКи;
            }
        }
    }
    ...
    public class A { } // <- класс также может быть публичным;
    class B
    {
        public int publicField;
        private int privateField;
        protected int protectedField;
        protected void Bar() { } // <- метод тоже может быть "защищенным";
        public void method()
        {
            Console.WriteLine(publicField); // -> "public" поле доступно, т.к. мы внутри класса;
            Console.WriteLine(privateField); // -> "private" поле доступно, т.к. мы внутри класса;
            Console.WriteLine(protectedField); // -> "protected" поле доступно, т.к. мы внутри класса;
        }
    }
    class C : B
    {
        ...
        {
            Console.WriteLine(publicField); // -> "public" поле доступно, оно на то и публичное;
            //Console.WriteLine(privateField); -> "private" поле недоступно, но оно есть, просто нельзя получить доступ из наследника;
            Console.WriteLine(protectedField); // -> "protected" поле доступно, т.е. благодаря этому модификатору...
            // Мы можем получать значения из "родительского" класса;
            // Тем временем "private" поля могут читаться и модифицироваться только внутри родительского класса;
        }
    }

    class DrBreensPrivateReserve
    {
        static void Main()
        {
            B b = new B();
            Console.WriteLine(b.publicField); // -> "public" поле доступно, т.к. мы хоть и изнутри класса, но нам позволено его видеть;
            // Console.WriteLine(b.privateField); -> "private" поле недоступно, что вполне логично, оно приватное;
            // Console.WriteLine(b.protectedField); -> "protected" поле недоступно, также недоступно, т.к. оно может быть доступно только в наследниках;
        }
    }
    ...
    class Human // <- это наш "человек";
    {
        public void Drive(Car car) // <- и он может водить машину;
        {
            car.Drive(); // <- вызываем метод нашей машины, который заставляет её "двигаться";
        }
    }
    ...
    class MickGordon
    {
        static void Main()
        {
            ...
            human.Drive(new Car()); // <- нашему человеку не важно, какая это машина;
            human.Drive(new SportCar()); // <- он может водить любую, потому что в них нет различий в управлении;
        }
    }

    class Car // Наша совершенно обычная машина;
    {
        public virtual void Drive() // <- обрати внимание на модификатор "virtual";
        {                           // Благодаря ему мы как бы говорим, что мы хотим, чтобы данный метод стал "виртуальным";
            StartEngine();          // Т.е. чтобы данный метод можно было переопределять в наследниках;
            Console.WriteLine("I am car and I am moving!"); // Машина, кстати, может ездить;
        }
        //private virtual void StartEngine() { } // <- мы не можем сделать виртуальным метод, к которому нельзя получить доступ извне;
        protected virtual void StartEngine() // <- в данном случае, благодаря модификатору "protected", мы можем получить доступ извне...
        {                                    // если это "извне" является только наследником данного класса;
            Console.WriteLine("Engine started");
        }
    }
    class SportCar : Car // <- класс "SportCar" является наследником класса "Car";
    {
        public override void Drive() // <- обрати внимание на модификатор "override";
        {                            // Благодаря этому модификатору мы как бы говорим, что мы хотим "переопределить" этот метод;
            base.Drive(); // <- мы даже можем вызвать в переопределенном методе все то, что выполнялось в переопределяемом методе родительского класса;
            Console.WriteLine("But I am moving really-really fast!"); // И да, эта машина конечно тоже ездит, но гораздо быстрее! И это действительно важно;
        } // При этом мы сохраняем имя метода, и нашему "человеку" не нужно переучиваться, чтобы водить спортивную машину, ведь это все равно машина!
        // Это, к слову, и зовется полиморфизмом;

        // public override void Drive(int a) { } <- мы не можем переопределить метод, которого нет;
        // В данном случае мы хотим принимать какие-то параметры, то мы строго обязаны соблюдать параметры...
        // переопределяемого метода. Если там что-то есть, тут тоже должно что-то быть;
        // Если же этого нет, то тут тоже ничего не должно быть. В противном случае это разные методы;

        //public void Drive() { } <- в случае, если мы хотим использовать метод с тем же именем, что и в родительском классе...
        // То будет вызываться только метод родительского класса, т.е. в данной практике нет смысла;

        protected override void StartEngine() // <- переопределяем метод с модификатором "protected", доступ к которому есть только у наследников;
        {
            base.StartEngine();
            Console.WriteLine("But engine started really fast!");
        }
    }
    ...
    abstract class Weapon // <- обрати внимание, что данный класс был создан с использованием ключевого слова "abstract";
    {                     // "Абстрактный" класс хранит некую идею и функционал, который могут перенимать его наследники;
        public void Fire() // <- допустим, нам нужно сделать так, чтобы все оружие могло стрелять;
        {                  // Нам не обязательно прописывать данный метод у каждого оружия, ведь мы можем реализовать это через родительский "абстрактный" класс;
            Console.WriteLine("Pew, pew!");
        }
        public abstract void Reload(); // <- мы можем также создать "абстрактный" метод внутри "абстрактного" класса;
                                       // Таким образом мы говорим, что у каждого наследника может быть свой собственный способ выполнения данного метода;
                                       // Обрати внимание, что у метода нет фигурных скобок "{}" и в конце стоит точка с запятой ";"
                                       // Мы не можем определить функционал абстрактного метода заранее, т.к. он все равно обязан переопределяться...
                                       // наследниками данного класса, т.е. в этом все равно нет смысла;
        public abstract int Damage { get; } // <- мы можем создать "абстрактную" переменную, чтобы у каждого оружия был свой урон;
        //public abstract int Damage; <- таким образом записывать не надо, иначе мы не сможем переопределить поведение переменной в наследниках;
    }

    class Rifle : Weapon // <- мы можем использовать весь функционал родительского абстрактного класса;
    {
        public override void Reload() // <- т.к. метод "Reload()" является "абстрактным" в родительском классе, мы ОБЯЗАНЫ его переопределить во всех наследниках;
        {                             // В противном случае наш код попросту не скомпилируется;
            Console.WriteLine("Reloading magazine with 30 bullets!"); // <- пишем собственную реализацию "абстрактного" метода;
        }
        public override int Damage { get { return 30; } } // <- мы задаем, что у нашей "винтовки" урон должен быть "30";
        // public override int Damage = 30; <- если бы мы переопределили переменную именно так, то при компиляции у нас происходила бы ошибка;
        // Обязательно используй { get; } или { get; set; } при переопределении поведения наследуемых "абстрактных" переменных;
        // Даже если для возвращения значения не нужна какая-то дополнительная логика;
    }
    class Pistol : Weapon
    {
        public override void Reload()
        {
            Console.WriteLine("Reloading magazine with 7 bullets!");
        }
        public override int Damage { get { return 12; } } // <- мы задаем, что у нашего "пистолета" урон должен быть "12";
    }
    ...
    class Player
    {
        ...
        {
            weapon.Fire(); // <- "стреляем" из оружия;
        }
        public void Reload(Weapon weapon)
        {
            weapon.Reload(); // <- "перезаряжаем" оружие;
        }
    }
    ...
    class HelloWorld
    {
        static void Main()
        {
            ...
            // Weapon weapon = new Weapon(); <- мы не можем создать экземпляр "абстрактного" класса;
            // В этом и состоит соль "абстракции" - мы понимаем, что если есть оружие, то оно может стрелять;
            // Мы описали данную идею в "абстрактном" классе и применили её к классам "Rifle" и "Pistol", т.к. нам необходимо, чтобы они могли "стрелять";
            Rifle rifle = new Rifle(); // <- мы можем создать экземпляр класса, который является наследником абстрактного класса;
            Weapon weapon = new Pistol(); // <- мы также можем создать переменную типа родительского класса;
            // Такой функционал будет полезен, если мы хотим реализовать "слот" для хранения оружия;
            // В большинстве игр, конечно, делают разные слоты для винтовки и пистолета. Однако в нашем случае нужно, чтобы в одном слоте...
            // мог храниться и пистолет, и винтовка. Поэтому мы назначаем общий тип "Weapon";
            player.Fire(rifle); // <- метод "Fire()" хранится в "абстрактном" классе, и мы можем использовать его в наследнике;
            player.Reload(rifle); // <- мы переопределили метод "Reload()", и у класса "Rifle" условная "перезарядка" должна быть своя;
        }
    }
    ...
    interface IDataProvider // <- вместо привычного нам "class" мы пишем "interface" и обычно принято, что имя "интерфейсов" начинается с "I"
        // Условно говоря, мы создаем интерфейс, благодаря мы можем получать данные из какого-то источника;
    {   // Для "интерфейса" не может быть создан экземпляр через, например, "IFirstInterface interface = new IFirstInterface()";
        // int a; <- мы не можем создавать поля (переменные) экземпляра внутри "интерфейса";
        string GetData(); // <- по факту, в "интерфейсах" стандартными модификаторами доступа для методов являются "public" и "abstract";
    }                     // Т.е. мы можем в любом случае выполнить метод извне, а так же ОБЯЗАНЫ переопределить его в наследниках;
    interface IDataProcessor // <- создаем условный интерфейс для обработки каких-то данных;
    {
        void ProcessData(IDataProvider dataProvider); // <- напоминаю, что в случае, если мы хотим наследовать данный интерфейс...
                                                      // Мы ОБЯЗАНЫ переопределить его методы, т.к. они по-дефолту являются "абстрактными";
                                                      // В этом и заключается суть - обращаясь к интерфейсу, мы уверены, что у его наследников...
                                                      // есть все нужные нам методы;
                                                      // Обрати внимание, что входным параметром для данного метода является какой-либо класс, который...
                                                      // наследует "интерфейс" "IDataProvider";
    }
    class ConsoleDataProcessor : IDataProcessor // <- наследуем "интерфейс", или говоря более профессиональным языком, "реализуем" "интерфейс";
    {
        public void ProcessData(IDataProvider dataProvider) // <- переопределяем метод, т.к. мы ОБЯЗАНЫ это сделать, и говорим, что во входных параметрах...
        {                                                   // нам необходимо иметь класс, который "реализует" "интерфейс" "IDataProvider";
            Console.WriteLine(dataProvider.GetData());      // В данном случае, для того чтобы "обработать" что-то, нам нужно что-то, откуда мы это что-то...
        }                                                   // можем взять;
    }
    class DbDataProvider : IDataProvider // <- мы реализуем интерфейс "IDataProvider";
    {
        public string GetData() // <- и, переопределяя метод наследуемого интерфейса, допустим, получая какие-то данные из какой-то базы данных;
        {                       // Кстати, заметь, что при "реализации интерфейсов" нам не нужно писать модификатор "override";
            return "Данные из БД"; // Иначе нам будет попросту выдавать ошибку;
        }
    }
    class FileDataProvider : IDataProvider
    {
        public string GetData() // <- в этом классе, который является наследником интерфейса "IDataProvider" мы, допустим, получаем данные из какого-то файла;
        {
    ...
    {
        public string GetData() // <- а здесь мы, допустим, получаем данные из какого-то API;
        {
    ...
    class MyProgram
    {
        static void Main()
        {
            IDataProcessor dataProcessor = new ConsoleDataProcessor(); // <- в данном случае нам нужен чисто функционал интерфейса IDataProcessor;
                                                                       // Создать экземпляр интерфейса мы не можем, поэтому мы создаем экземпляр...
                                                                       // класса, который этот интерфейс реализует;
            dataProcessor.ProcessData(new DbDataProvider()); // Из этого интерфейса нам крайне необходим метод "ProcessData" чтобы, допустим, обработать данные;
                                                             // Входным параметром для этого метода, как мы определили ранее, должен быть класс...
            // который реализует интерфейс "IDataProvider". В этой строчке мы создаем класс "DbDataProvider", который как раз этот интерфейс и реализует;
            // Таким образом, мы гарантируем себе, что вызывая метод "ProcessData", входным параметром будет тот класс, который реализует интерфейс "IDataProvider"...
            // из которого нам жизненно необходим метод "GetData()", чтобы "получить" данные и их "обработать";
            // Напомню, что в классах, которые реализуют интерфейс, все методы интерфейса ОБЯЗАНЫ быть переопределены;
            // Я так часто напоминаю об этом для того, чтобы ты понял, для чего функционал интерфейсов вообще необходим;
            dataProcessor.ProcessData(new FileDataProvider()); // Класс "FileDataProvider" также реализует интерфейс "IDataProvider" и, соответственно, у него...
                                                               // ОБЯЗАН быть метод "GetData()", который необходим для выполнения метода "ProcessData";
            dataProcessor.ProcessData(new APIDataProvider());
            // Честно говоря, интерфейсы оч крутая штука, но для того, чтобы понять, для чего они нужны, наверное, нужно смотреть РЕАЛЬНЫЕ примеры их применения;
            // Т.к. я ограничен в написании действительно хороших примеров, приходится обходиться условными. Поэтому, чтобы понять функционал интерфейсов...
            // Я рекомендую найти какие-нибудь гайды по этой теме или посмотреть видео, где эта тема подробно разъясняется на действительно хороших примерах;
            // Просто знай, что данный функционал есть, а если ты понял из моей писанины для чего это нужно, то ты просто гений;
        }
    }
}

namespace ConsoleApp3 // <- я специально создал отдельное пространство имен, чтобы в знакомом для вас уже примере наглядно показать, как можно использовать интерфейсы;
{                     // Крайне рекомендую смотреть на пример ниже, а также пример выше (это где игрок, оружие и абстрактные классы) и активно сравнивать различия в них;
    interface IWeapon // <- мы создаем общий интерфейс для всего оружия, т.к. хотим, чтобы у каждого оружия...
    {
        int Damage { get; } // был свой собственный урон;
    }
    interface IFirearm : IWeapon // <- создаем интерфейс отдельно для огнестрельного оружия;
    {
        void Fire(); // <- т.к. каждый огнестрел должен уметь именно стрелять...
        void Reload(); // <- и перезаряжаться;
    }
    interface IMeleeWeapon : IWeapon // <- создаем интерфейс для оружия ближнего боя;
    {
        void Stab(); // <- каждое оружие ближнего боя должно уметь делать дырки в теле;
    }
    interface IFirearmWithMelee : IFirearm, IMeleeWeapon // <- мы хотим создать огнестрельное оружие, которое совмещает функционал и огнестрела...
    {                                                    // и оружия ближнего боя. Для той же винтовки со штык-ножом;
        // Заметь, что наш интерфейс смог стать наследником аж ДВУХ других интерфейсов - это еще одна особенность интерфейсов, в отличие от классов...
        // т.к. один класс может унаследовать только один другой класс;
        int MeleeDamage { get; } // <- т.к. мы не хотим объединять урон в ближнем и дальнем бою, для этого интерфейса мы создаем специальное поле для того...
        // чтобы указывать урон именно при применении в ближнем бою;
    }
    class Rifle : IFirearm // <- наша винтовка реализует интерфейс для огнестрельного оружия...
    {
        public void Fire() { Console.WriteLine("Pew-pew-pew-pew-pew-pew-pew!"); } // Значит наша винтовка должна уметь стрелять...
        public void Reload() { Console.WriteLine("Reloading magazine with 30 bullets!"); } // Перезаряжаться...
        public int Damage { get { return 30; } } // И т.к. интерфейс огнестрельного оружия является наследником интерфейса для всего оружия...
                                                 // то мы должны указать урон для нашей винтовки;
    }
    class Pistol : IFirearm
    {
        public void Fire() { Console.WriteLine("Pew!"); }
        public void Reload() { Console.WriteLine("Reloading magazine with 7 bullets!"); }
        public int Damage { get { return 12; } }
    }
    class Knife : IMeleeWeapon // <- это нож, и он реализует интерфейс для оружия ближнего боя;
    {
        public void Stab() { Console.WriteLine("Wshooh and you have hole!"); } // Соответсвенно, наш нож должен уметь делать дырки;
        public int Damage { get { return 100; } } // И, т.к. интерфейс для оружия ближнего боя является наследником интерфейса для всего оружия...
                                                  // то мы должны указать урон для нашего ножа;
    }
    class RifleWithBayonet : Rifle, IFirearmWithMelee // <- Мы наследуем класс винтовки, чтобы не задавать снова "Fire()" и "Reload()", т.к. понятное дело...
    {                                                 // что винтовка со штыком должна уметь стрелять и перезаряжаться так же, как и обычная винтовка;
                                                      // Т.е. помимо того, что мы наследуем интерфейс огнестрельного оружия, мы также наследуем уже...
                                                      // переопределенные методы данного интерфейса у класса винтовки;
                                                      // Причем мы можем реализовывать сразу несколько интерфейсов;
        public void Stab() { Console.WriteLine("Wshooh and you have hole!"); } // <- учим нашу винтовку со штыком делать дырки... 
        public int MeleeDamage { get { return 100; } } // <- и т.к. мы реализуем отдельный интерфейс для огнестрельного оружия со встроенным оружием ближнего боя...
                                                       // То нам необходимо указать урон при использовании винтовки именно в ближнем бою, т.к. обычный урон...
    }                                                  // наносится конкретно при стрельбе из оружия;
    class Player
    {
        public void Fire(IFirearm weapon) { weapon.Fire(); } // <- игрок должен уметь стрелять;
        public void Reload(IFirearm weapon) { weapon.Reload(); } // <- игрок должен уметь перезаряжаться;
        public void Stab (IMeleeWeapon weapon) { weapon.Stab(); } // <- игрок должен уметь делать дырки;
    }
    ...
    class HelloWorld
    {
        ...
        {
            Player player = new Player();
            IFirearm rifle = new Rifle(); // <- нам нужен функционал винтовки именно как огнестрельного оружия;
            IFirearm pistol = new Pistol(); // <- то же самое и с пистолетом, ведь нам надо делать пиу-пиу;
            IMeleeWeapon knife = new Knife(); // <- здесь нам нужен функционал ножа именно как оружия ближнего боя;

            player.Fire(rifle); // <- наша винтовка стреляет!
            player.Reload(rifle); // <- наша винтовка перезаряжается;

            player.Fire(pistol);
            player.Reload(pistol);

            player.Stab(knife); // <- наш нож делает дырки!

            IFirearmWithMelee rifle_w_bayonet = new RifleWithBayonet(); // <- здесь нам нужен функционал и огнестрельного оружия, и оружия ближнего боя...
                                                          // Для таких моментов мы создали отдельный интерфейс, который как раз и объединяет эти две вещи;
            player.Stab(rifle_w_bayonet); // <- наша винтовка со штыком может делать дырки;
            player.Fire(rifle_w_bayonet); // <- наша винтовка со штыком может стрелять;
            player.Reload(rifle_w_bayonet); // <- наша винтовка со штыком может перезаряжаться;

            IFirearm rifle_w_bayonet2 = new RifleWithBayonet(); // <- в случае, если нам нужен функционал винтовки со штыком только как огнестрельного оружия...
                                                // мы можем указать в типе переменной интерфейс, который реализует ТОЛЬКО функционал огнестрельного оружия;
            // player.Stab(rifle_w_bayonet2); <- однако в таком случае наша винтовка со штыком теперь не сможет делать дырки;
            player.Fire(rifle_w_bayonet2); // <- но может стрелять...
            player.Reload(rifle_w_bayonet2); // <- и перезаряжаться;

            IMeleeWeapon rifle_w_bayonet3 = new RifleWithBayonet(); // <- в случае, если нам нужен функционал винтовки со штыком только как оружия ближнего боя...
                                               // мы можем указать в типе переменной интерфейс, который реализует ТОЛЬКО функционал оружия ближнего боя;
            player.Stab(rifle_w_bayonet3); // <- в таком случае наша винтовка со штыком может делать дырки;
            // player.Fire(rifle_w_bayonet3); <- но не может стрелять...
            // player.Reload(rifle_w_bayonet3); <- и перезаряжаться;

            IWeapon[] inventory = new IWeapon[] { rifle, pistol, knife };  // <- можем условно создать инвентарь, или же массив, в котором...
            // будем хранить все наше оружие. Обрати внимание, что типом массива является интерфейс "IWeapon", который является общим для всего оружия;
        }
    }
    // Надеюсь, пример выше был более наглядным для того, чтобы понять суть интерфейсов. Внимательно изучи код, не только читай комментарии.
    // И обязательно сравни все это с предыдущей реализацией, которая находится в главе с абстрактными классами, ведь там мы реализовывали точно такой же функционал;
    // Только здесь мы еще и умудрились реализовать винтовку со штык-ножом, что очень круто, как по мне;
    ...
    {
        void Action(); // <- обрати внимание...
    }
    interface ISecondInterface
    {
        void Action(); // <- что у двух наших интерфейсов имеется метод с одинаковым названием;
    }
    class MyNewClass : IFirstInterface, ISecondInterface // <- в данном классе мы реализовываем сразу два интерфейса;
    {
        public void Action() // <- буквально одним методом мы реализовываем и интерфейс "IFirstInterface", и интерфейс "ISecondInterface";
        {                    // И нам не будет выдавать ошибку, т.к. мы таким образом переопределяем методы сразу двух интерфейсов;
            Console.WriteLine("Action!");
        }
    }
    class AnotherNewClass : IFirstInterface, ISecondInterface
    {
        void IFirstInterface.Action() // <- можно переопределить метод для каждого интерфейса отдельно;
        {
            Console.WriteLine("First action!");
        }
        void ISecondInterface.Action() // <- можно переопределить метод для каждого интерфейса отдельно;
        {
    ...
    class Overwatch
    {
        static void Main()
        {
            MyNewClass newClass = new MyNewClass(); // <- создаем экземпляр класса, в котором мы переопределяем метод для двух интерфейсов сразу;
            Foo(newClass); // -> выведет в консоль сообщение "Action!", через этот метод мы обращаемся к интерфейсу "IFirstInterface";
            Bar(newClass); // -> выведет в консоль сообщение "Action!", через этот метод мы обращаемся к интерфейсу "ISecondInterface";

            AnotherNewClass anotherClass = new AnotherNewClass(); // <- создаем экземпляр класса, в котором переопределяем метод для каждого интерфейсов отдельно;
            Foo(anotherClass); // -> выведет в консоль сообщение "First action!", через этот метод мы обращаемся к интерфейсу "IFirstInterface";
            Bar(anotherClass); // -> выведет в консоль сообщение "Second action!", через этот метод мы обращаемся к интерфейсу "ISecondInterface";

            // Более сокращенный вариант без каких-либо дополнительных методов, в котором мы просто приводим переменную к нужному интерфейсу;
            ((IFirstInterface)anotherClass).Action(); // -> выведет в консоль сообщение "First action!", через этот метод мы обращаемся к интерфейсу "IFirstInterface";
            ((ISecondInterface)anotherClass).Action(); // -> выведет в консоль сообщение "Second action!", через этот метод мы обращаемся к интерфейсу "ISecondInterface";


        }
        static void Foo(IFirstInterface firstInterface) // <- в данном случае, т.к. класс "AnotherNewClass" имеет два переопределения...
        { // для каждого из своих интерфейсов отдельно, мы можем принимать, допустим, в новом методе какой-то определенный интерфейс;
            firstInterface.Action(); // -> в данном случае мы увидим в консоли сообщение "First action!"...
        }                            // т.к. мы обращаемся именно к интерфейсу "IFirstInterface" нашего класса;
        static void Bar(ISecondInterface secondInterface)
        {
            secondInterface.Action(); // -> здесь же мы обращаемся именно к интерфейсу "ISecondInterface" нашего класса;
        }                             // И, соответственно, видим сообщение "Second action!";
                                      // Заметь, что в методе "Main()" чуть выше мы вставляли в эти два метода один и тот же экземпляр класса...
                                      // который, в свою очередь, был наследником сразу двух интерфейсов;
    }
    ...
    class NewProgram
    {
        static void Main()
        {
            StructPoint s_point = new StructPoint();
            s_point.x = 1; s_point.y = 2;
            // Особенности использования структур:
            // 1. Работа со структурами обычно в разы быстрее, т.к. данные храняться в "стеке";
            // 2. Значения храняться, по сути, уже в переменной;
            // 3. Стек быстро очищается, после того как значения больше не нужны, вследствие этого очистка оперативной памяти происходит моментально;
            // ОДНАКО не забывай, что память стека ограничена;
            // 4. При использования структуры как входного параметра в методе, ну т.е. "static void Method(StructPoint point) { } "...
            // все данные из структуры копируются в совершенно новый экземпляр, что вызывает увеличение использования памяти стека;
            // И т.к. это совершенно новый экземпляр, мы не сможем взаимодействовать с оригинальным экземпляром, изменяя значения полей и т.д.;
            // К слову, подобные манипуляции с копированием неплохо так замедляют выполнение кода;

            ClassPoint c_point = new ClassPoint();
            c_point.x = 1; c_point.y = 2;
            // Особенности использования классов:
            // 1. Работа с классами обычно медленнее, т.к. для создания экземпляра нужно найти место в куче, разместить там данные и получить обратно ссылку;
            // 2. В переменной мы храним ссылку на место в оперативной памяти, в котором находится экземпляр нашего класса;
            // 3. Как только класс перестал быть нужен, необходимо ждать "сборщика мусора", чтобы очистить оперативную память;
            // 4. При использования класса как входного параметра в методе, ну т.е. "static void Method(ClassPoint point) { } "...
            // мы используем лишь ссылку на экземпляр этого класа в управляемой куче, новых экземпляров не создается, т.к. внутри метода мы будем...
            // обращаться конкретно к месту нашего класса в куче. Именно поэтому внутри метода мы можем свободно изменять значения полей экземпляра класса;
            // В таких случаях использование классов будет быстрее;
        }
    }
    ...
    class HowGenericsWorks
    {
        static void Swap(ref int a, ref int b) // <- обычный метод для перестановки значений у двух переменных местами;
        {
            int temp = a;
            a = b;
            b = temp;
        }
        static void Swap(ref double a, ref double b) // <- как видишь, если мы хотим использовать другой тип данный, например "double" вместо "int"...
        { // Нам приходится делать перегрузку;
            double temp = a;
            a = b;
            b = temp;
        }
        // Однако если мы хотим, чтобы мы могли менять значения переменных у самых разных типов, и дабы не писать для каждого типа свою перегрузку...
        // мы можем использовать обощение, или же "generic";
        static void SwapGeneric<T>(ref T a, ref T b) // <- обрати внимание на "<T>", именно благодаря ему мы используем в данном методе обобщение;
        { // Вместо "<T>" может быть и "<anyOtherName>", однако принято писать "<T>" или "<TAnyOtherName>" чтобы сразу было видно, что это обобщение;
            T temp = a;
            a = b;
            b = temp;
        }
        ...
        {
            int a = 5; int b = 7;
            Swap(ref a, ref b); // a = 7, b = 5 | <- все работает;
            SwapGeneric(ref a, ref b); // a = 5, b = 7 | <- также работает;
            // ОДНАКО!
            double c = 3.7; // <- если мы создадим переменную типа "double"...
            // SwapGeneric(ref a, ref c); <- и попробуем использовать обобщение, то компилятор не даст нам этого сделать;
            // Так как по факту в обобщении мы используем параметры именно одного типа, типа обобщения "<T>";
            double d = 7.2;
            SwapGeneric(ref c, ref d); // <- как видишь, когда переменные у нас одинакового типа, обобщение сразу заработало;
            SwapGeneric<double>(ref c, ref d); // <- мы также можем явно указать типизацию, но в данном случае в этом нет смысла;

            int defaultInt = ReturnDefault<int>(); // <- в данном случае, т.к. у нас нет входных параметров, а обобщению необходимо понимать...
            // с каким конкретно типом данных ему нужно работать, то мы используем явную типизацию, т.е. указываем "<int>";

            List<int> list = new List<int>(); // <- "List" является, по факту, массивом с динамическим размером;
            // Как видишь, чтобы создать такой лист, нам необходимо использовать обобщение, чтобы тот понимал, данные какого типа он будет содержать;
            list.Add(a); // <- добавляем в конец листа значение нашей переменной "a", т.е. "5";
            list.Add(b); // <- добавляем в конец листа значение нашей переменной "b", т.е. "7";
            Console.WriteLine(list[0]); // <- показываем значение индекса "0". Т.к. первым значением мы туда вставили значение "a", то нам высветит "5";

            ArrayList arrayList = new ArrayList(); // <- если мы хотим лист без строгой типизации, то мы можем использовать "ArrayList";
            arrayList.Add(5); // <- как видишь, сюда входят и данные типа "int"...
            arrayList.Add("55"); // <- и данные типа "string";
        }

        static T ReturnDefault<T>()
        {
            return default(T); // <- возвращает стандартное значение для данного типа данных;
        }                      // Например, для типа "int" стандартным значением будет являться "0", а для "string" - "null";
    }
}
